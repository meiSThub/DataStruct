...
public class SHA {
	...
	//byteData是一个长度为4的字节数组
	//而一个int值在java中就是4个字节，所以可以用一个int存储一个长度为4字节数组
	//byteData[i]&0xff)<<24  就是把下标为i的字节存放到int的最高位，即最左边的1个字节
    public static int byteArrayToInt(byte[] byteData,int i){
        //0a 0b 0c 0d  24       16       8       0
        //         0a000000  or  0b0000  or   0c00  or   0d
        //            0a0b0c0d
		//把字节数组中的每个字节放到int的对应的字节位置上，在合并成一个int
        return ((byteData[i]&0xff)<<24)|((byteData[i+1]&0xff)<<16)|((byteData[i+2]&0xff)<<8)|(byteData[i+3]&0xff);
    }
	
	//这个方法就跟上面的方法做相反的操作，即把一个int值按照字节拆分成一个长度为4的字节数组
